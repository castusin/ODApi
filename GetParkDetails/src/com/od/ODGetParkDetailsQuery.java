...
/*select pv.Park_id, pv.Room_type_code, count(*) , min(Current_price) as Current_price,p.Facility_type,p.Room_type_desc,p.Max_people,p.Max_adults,p.Max_kids,p.Max_qty,p.Regular_price,p.Facility_image,p.Facility_in_Room_facilities,p.Facility_type_title,p.Facility_type_code,A.Park_sub_images
from Park_current_pricing_availability pv,Park_facility_inventory_pricing p, Park_info A 
where p.park_id = pv.Park_id and p.facility_type_code = pv.Room_type_code and p.park_id = A.park_id and pv.date between ? and ? and current_available_qty > 0 and pv.park_id=?	
group by A.Park_sub_images, pv.Park_id, pv.Room_type_code,p.Room_type_desc,p.Max_people,p.Max_adults,p.Max_kids,p.Max_qty,p.Regular_price,p.Facility_image,p.Facility_in_Room_facilities,p.Facility_type_title,p.Facility_type_code having count(*)=? 
union
select '',p.Park_id, p.Facility_type_code, 0 , 0 , p.Facility_type,p.Room_type_desc,p.Max_people,p.Max_adults,p.Max_kids,p.Max_qty,p.Regular_price,p.Facility_image,p.Facility_in_Room_facilities,p.Facility_type_title,p.Facility_type_code
from Park_facility_inventory_pricing p
where p.park_id=? and Facility_type='O'*/
